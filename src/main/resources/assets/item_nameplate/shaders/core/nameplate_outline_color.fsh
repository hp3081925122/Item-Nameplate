...
void main() {
    ...
    vec4 center = atlasPixel.x >= glyphBounds.x && atlasPixel.x < glyphBounds.y && atlasPixel.y >= glyphBounds.z && atlasPixel.y < glyphBounds.w
    ...
    for (int y = -1; y <= 1; ++y) {
        for (int x = -1; x <= 1; ++x) {
            ...
            if (samplePixel.x >= glyphBounds.x && samplePixel.x < glyphBounds.y && samplePixel.y >= glyphBounds.z && samplePixel.y < glyphBounds.w) {
                outline = max(outline, texture(Sampler0, texCoord0 + vec2(x, y) * pixel).a);
            }
        }
    }

    vec4 color = center.a >= 0.1
        ? vec4(center.rgb * vertexColor.rgb, center.a * vertexColor.a)
        : vec4(0.0, 0.0, 0.0, outline * vertexColor.a);
